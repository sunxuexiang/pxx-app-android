...
export default class GoodsShare extends Component {
  ...
  render() {
    const { showShare, changeShowShare, checkedSku } = this.props.relaxProps;
    // 社交电商相关内容显示与否
    const social = true;
  ...
  shareToMoments = () => {
    ...
    share.shareToMoments({
      url: `${h5Url}/pages/package-B/goods/goods-details/index?skuId=${goodsInfo.get('goodsInfoId')}`,
      title: `${goodsInfo.get(
        'goodsInfoName'
      )}，我发现了一件好货，赶快来看看吧...`,
  ...
  copy = async () => {
    ...
    Clipboard.setString(
      `${h5Url}/pages/package-B/goods/goods-details/index?skuId=${goodsInfo.get('goodsInfoId')}`
    );
