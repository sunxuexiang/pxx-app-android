...
export default class AppStore extends Store {
  ...
  init = async (id) => {
    ...
    const res = await webApi.details(id);
    if (res.code == 'K-000000') {
      this.dispatch('record:init', res.context.tradeWareHouse);
      this.dispatch('record:address', res.context.tradeWareHouse.addressDetail);
      this.dispatch('record:order', res.context);
