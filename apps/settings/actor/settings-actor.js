...
export default class SettingsActor extends Actor {
   ...
   */
  @Action('init')
  init(state, { customer, shareInfo }) {
    return state.set('customer', customer).set('shareInfo', shareInfo);
  }

  /**
   * 修改分享弹框可见性
   */
  @Action('change:modal:visible')
  changeModelVisible(state, flag) {
    return state.set('modelVisible', flag);
  }

  /**
   * 图文分享弹框显示隐藏
   */
  @Action('change:img:share:visible')
  changeImgShareVisible(state, flag) {
    return state.set('modelVisible', false).set('imgShareVisible', flag);
  }

  /**
   * 版本更新信息
   */
  @Action('set: upgrade: info')
  setUpgradeInfo(state, upgradeInfo) {
    return state.set('upgradeInfo', fromJS(upgradeInfo));
  }
  ...
  toggleShowPrivacyPolicyAgreement(state) {
    return state.set('toggleShowPrivacyPolicyAgreement', !state.get('toggleShowPrivacyPolicyAgreement'));
  }
  ...
  registerContent(state, res ) {
    return state.set('registerContent', res);
  }

  @Action('agree:privacyPolicyContent')
  privacyPolicyContent(state, res) {
    return state.set('privacyPolicyContent', res);
  }
}
