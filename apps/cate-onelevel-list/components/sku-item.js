...
export default class SkuItem extends React.Component {
  ...
  small = () => {
    ...
    //预约活动
    const appointmentSaleVOList = [];
    const bookingSaleVOList = [];
...
console.log('goodsInfo=====>',goodsInfo)
            ...
            <View style={styles.tagCon}>
              ...
              {!buyPoint && iepShowFlag && (
                <View style={styles.iepBoard}>
                  <Text
                    allowFontScaling={false}
                    style={[styles.iepText, { backgroundColor: mainColor }]}
                  >
                    {enterprisePriceName}
                  </Text>
                </View>
              ...
              !iepShowFlag && (
                <View style={styles.socialBtn}>
                  ...
                  >
                    <Text
                      allowFontScaling={false}
                      style={[styles.btnText1, { color: mainColor }]}
                    >
                      发圈
                    </Text>
                  </TouchableOpacity>
  ...
  _afterChangeNum = (num) => {
    ...
    this.props.updatePurchaseCount();
  };
  /**
   * 存储推荐列表已阅读的商品信息
   *
