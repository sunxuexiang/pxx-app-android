...
export default class GoodsShare extends Component {
  ...
  render() {
    const { showShare, changeShareModal, goodsInfo } = this.props.relaxProps;
    // 社交电商相关内容显示与否
    const social = true;
  ...
  shareToMoments = async () => {
    const { goodsInfo, images, h5Url, goods } = this.props.relaxProps;
    ...
    if (WMkit.isDistributor()) {
      ...
      url = `${h5Url}/shop-index/goods-detail/${customerId}/${goods.get(
        'goodsId'
   ...
   */
  copy = async () => {
    const { goodsInfo, h5Url, goods } = this.props.relaxProps;
    if (!h5Url) return;
    ...
    if (WMkit.isDistributor()) {
      ...
      const customerId = JSON.parse(loginData).customerId;
      Clipboard.setString(
        `${h5Url}/shop-index/goods-detail/${customerId}/${goods.get(
          'goodsId'
        )}/${goodsInfo.get('goodsInfoId')}`
      );
    }
