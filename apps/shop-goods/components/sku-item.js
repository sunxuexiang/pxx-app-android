...
export default class SkuItem extends React.Component {
  static relaxProps = {
    addDistributorGoods: noop,
    delDistributorGoods: noop
  };
  ...
  small = () => {
    ...
    const { addDistributorGoods, delDistributorGoods } = this.props.relaxProps;
    // 是否是分销员
        ...
        <View style={styles.content}>
          ...
          <View style={styles.row}>
            ...
            <View style={styles.socialBtn}>
              {goodsInfo.joinDistributior == 1 ? (
                <TouchableOpacity
                  style={[styles.btnBox1, { borderColor: mainColor }]}
                  activeOpacity={0.8}
                  onPress={async () => {
                    await delDistributorGoods(goodsInfo.goodsInfoId);
                    this.setState((preState) => {
                      preState.goodsItem.goodsInfo.joinDistributior = 0;
                      return preState;
                    });
                  }}
                >
                  <Text style={[styles.btnText1, { color: mainColor }]}>从店铺删除</Text>
                </TouchableOpacity>
              ) : (
                <TouchableOpacity
                  style={[
                  ...
                  activeOpacity={0.8}
                  onPress={async () => {
                    await addDistributorGoods(
                      goodsInfo.goodsId,
                      goodsInfo.goodsInfoId,
                      goodsInfo.storeId
                    );
                    this.setState((preState) => {
                      preState.goodsItem.goodsInfo.joinDistributior = 1;
                      return preState;
                    });
                  }}
                >
                  <Text style={styles.btnText2}>加入店铺</Text>
                </TouchableOpacity>
              )}
            </View>
          </View>
