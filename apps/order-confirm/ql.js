...
export const OrderMaxPointQL = QL('OrderMaxPointDiscountQL', [
  ...
  (
    ...
    if (stores&&stores.size>0){
      let specialPrice=Number(0);
      stores.forEach(param=>{
        const goodsInfo =param.get('tradeItems').filter((param) => param.get('goodsInfoType') == 1);
        goodsInfo.forEach(goods=>{
          specialPrice=_.add(specialPrice,_.mul(goods.get('specialPrice'),goods.get('num')));
        })
      })
      totalPrice = _.sub(totalPrice, specialPrice);
    }
