...
export default class AppStore extends Store {
  bindActor() {
    return [new DescActor(), new GoodsActor(), new CouponActor(), new GrouponActor()];
  }
  ...
  init = async (skuId) => {
    ...
    if (spuRes.code === config.SUCCESS_CODE) {
      ...
      this.transaction(() => {
        ...
        this.dispatch('groupon-detail: info', spuRes);
  ...
  */
  ...
  /**
     * 获取服务时间
     */
  queryServerTime = async () => {
    const serverTimeNew = await webApi.queryServerTime();
    //存储服务时间
    this.dispatch('groupon:detail:serverTime', serverTimeNew);
  }

  /**公告请求 */
  getGrouponLatestInstanceByActivityId = async () => {
    const grouponActivityId = this.state().getIn(['grouponActivity', 'grouponActivityId']);
    const { code, context } = await webApi.getGrouponLatestInstanceByActivityId(grouponActivityId);
    if (
      code == config.SUCCESS_CODE &&
      context.grouponInstance) {
      if (context.customerName != '') {
        if (this.state().get('oldGroupNo') != context.grouponInstance.grouponNo) {
          this.dispatch('groupon:notice:grouponInst', context);
          this.dispatch('groupon:notice:oldGroupNo', context.grouponInstance.grouponNo);
        }
        else {
          this.dispatch('groupon:notice:grouponInst', {});
        }
      } else {
        this.dispatch('groupon:notice:grouponInst', {});
      }
