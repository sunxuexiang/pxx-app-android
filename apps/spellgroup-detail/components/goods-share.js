...
export default class GoodsShare extends Component {
  ...
  render() {
    ...
    // 社交电商相关内容显示与否
    const social =
      isDistributor && goodsInfo.get('distributionGoodsAudit') == '2';
    return (
      showShare && (
  ...
  shareToMoments = async () => {
    ...
    if (WMkit.isDistributor()) {
      ...
      url = `${h5Url}/pages/package-B/goods/group-details/index?skuId=${goodsInfo.get(
        'goodsInfoId'
      )}&channel=mall&inviteeId=${customerId}`;
    } else {
      url = `${h5Url}/pages/package-B/goods/group-details/index?skuId=${goodsInfo.get(
        'goodsInfoId'
      )}`;
  ...
  copy = async () => {
    ...
    if (WMkit.isDistributor()) {
      ...
      const customerId = JSON.parse(loginData).customerId;
      Clipboard.setString(
        `${h5Url}/pages/package-B/goods/group-details/index?skuId=${goodsInfo.get(
          'goodsInfoId'
        )}&channel=mall&inviteeId=${customerId}`
      );
    } else {
