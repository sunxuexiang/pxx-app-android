...
import { config, WMkit, VAS } from 'wmkit';
import { evaluateWebapi } from 'wmkit/biz';
...
export default class AppStore extends Store {
  ...
  changeSpecialVisible = (value) => {
    msg.emit('app:bottomVisible', { key: 'GoodsList', visible: !value });
    this.dispatch('goodsActor:changeSpecialVisible', value);
  };
    ...
    if (code === config.SUCCESS_CODE) {
      ...
      if (goodsInfos && goodsInfos.length > 0) {
        ...
        this.transaction(() => {
          ...
          const specialFlag = goodsInfos[0] && goodsInfos[0].goodsInfoType == 1;
          if(specialFlag){
            this.changeSpecialVisible(true);
          } else if (context.goods.saleType === 0) {
