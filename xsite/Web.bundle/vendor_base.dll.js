...
exports.msg = exports.util = exports.promise = exports.log = exports.optimizeImage = undefined;
...
var _urlHelper = __webpack_require__(225);

var _urlHelper2 = _interopRequireDefault(_urlHelper);

var _asynLoaderUtil = __webpack_require__(226);

var _asynLoaderUtil2 = _interopRequireDefault(_asynLoaderUtil);

var _displayRuleUtil = __webpack_require__(252);

var _displayRuleUtil2 = _interopRequireDefault(_displayRuleUtil);

var _imageUtil = __webpack_require__(253);

var imageUtil = _interopRequireWildcard(_imageUtil);

var _xsiteMsg = __webpack_require__(256);
...
var util = {
    urlHelper: _urlHelper2.default,
    imageUtil: imageUtil,
    asynLoaderUtil: _asynLoaderUtil2.default,
    displayRuleUtil: _displayRuleUtil2.default
}; /**
    * @desc
    *  做为一个工具类 只引用最基本最简单的...不要把react等信息加进来
    *
    *  使用webpackexternal
    *
    *  import  xKit from  'xKit';
    *  或
    *  import  {log} from  'xKit' ;
    * @使用场景
    *
    * @author  杨晓东[of1081]
    * @company qianmi.com
...
var urlHelperTips = function () {
    console.log("urlHelper,不再放到这个位置了,请引用 xKit.util.urlHelper");
};
var asynLoaderUtilTips = function () {
    console.log("asynLoaderUtil,不再放到这个位置了,请引用 xKit.util.asynLoaderUtil");
};
var optimizeImage = imageUtil;
exports.default = {
    optimizeImage: optimizeImage,
    log: _log2.default,
    msg: _xsiteMsg.msg,
    promise: _promise2.default,
    urlHelper: urlHelperTips,
    asynLoaderUtil: asynLoaderUtilTips,
    util: util
};
...
exports.default = function () {
    xIS_DEBUG && console && console.log && console.log.apply(console, arguments);
};

var xIS_DEBUG = window['xIS_DEBUG'] = false;
/**
 * 判断当前的环境
 */
/**
 * @desc
 * import log from  'xKit/common/log';
 *
 * @使用场景
 *
 * @author  杨晓东[of1081]
...
if (ctx && !ctx.xGod) {
    ctx.xGod = function () {
        xIS_DEBUG = true;
        console.log('xKit : "开启日志记录"');
    };
    if (location && location.hash && location.hash.indexOf('#debug') >= 0) {
        xIS_DEBUG = true;
        console.log('xKit : "开启日志记录"');
    }
}
...
var WidgetCenter = function () {
    function WidgetCenter() {
        (0, _classCallCheck3.default)(this, WidgetCenter);
    }

    (0, _createClass3.default)(WidgetCenter, null, [{
        key: "getRepo",
        ...
        key: "registerWidget",
        value: function registerWidget(widget, _ref) {
            var isProxy = _ref.isProxy,
                widgetNameSpace = _ref.widgetNameSpace,
...
var AComp = /** @class */function (_super) {
    __extends(AComp, _super);
    function AComp(props) {
        return _super.call(this, props) || this;
    }
    AComp.prototype.render = function () {
        var target = this.props.target;
        if (typeof target === "boolean") {
            target = target ? "_blank" : "_self";
        }
        return React.createElement("a", __assign({}, this["props"], { target: target }), this.props.children);
    };
    AComp.defaultProps = {
...
var config_order_util_1 = __webpack_require__(147);
var widget_center_1 = __webpack_require__(125);
var $script = __webpack_require__(222);
var x_site_kit_1 = __webpack_require__(50);
var img_comp_1 = __webpack_require__(258);
var img_comp_webp_1 = __webpack_require__(265);
var business_data_center_1 = __webpack_require__(266);
//如果是第三方系统 , 那么可以使用webpack external来表示,或者直接从window中取 ..
window["xDllContext"] = __assign({}, window["xDllContext"], { BusinessDataCenter: business_data_center_1.default,
    widgetCenter: widget_center_1.default,
    configOrderUtil: config_order_util_1.default,
    xKit: x_site_kit_1.default,
    $script: $script, iflux: { msg: x_site_kit_1.msg }, msg: x_site_kit_1.msg, commonComp: {
        //公共的一些组件;;
        xImage: img_comp_1.default,
        xImageWebp: img_comp_webp_1.default
    } });
 ...
 **/
var React = __webpack_require__(40);
var immutable_1 = __webpack_require__(220);
var widget_center_1 = __webpack_require__(125);
var output_generate_1 = __webpack_require__(221);

var ConfigOrderUtil = function () {
    function ConfigOrderUtil() {
        (0, _classCallCheck3.default)(this, ConfigOrderUtil);
    }

    (0, _createClass3.default)(ConfigOrderUtil, null, [{
        key: "generateCompTree",

        /**
         * 支持编辑时生成
         * 支持生成页面使用 只有组件 没有组件-p;
         *
         * @param configOrder
         * @param pathArray
         * @param propsToChildren
         * @param isShowFull
         * @returns {DOMElement<P, T>}
         */
        value: function generateCompTree(_ref) {
            var configOrder = _ref.configOrder,
                pathArray = _ref.pathArray,
                propsToChildren = _ref.propsToChildren,
                isShowFull = _ref.isShowFull,
                isEditModel = _ref.isEditModel;

            var _widget_center_1$defa = widget_center_1.default.getRepo(),
                widgetProxyRep = _widget_center_1$defa.widgetProxyRep,
                widgetRep = _widget_center_1$defa.widgetRep;

            var widgetTreeConfig = configOrder;
            var widgetName = widgetTreeConfig.get("widgetNameSpace");
            if (!pathArray) {
                pathArray = immutable_1.List([]);
            }
            if (!widgetName) {
                throw new Error("组件为空");
            }
            var proxyComp = widgetProxyRep[widgetName],
                Comp = widgetRep[widgetName],
                props; //用于生成配置，信息
            if (widgetTreeConfig.has("props")) {
                if (isEditModel) {
                    if (!proxyComp) {
                        throw new Error("组件对应配置[p.js]未加载到widgetProxyRep中或用户配置清单版本较老,widgetName::" + widgetName);
                    }
                    if (proxyComp.prototype.initServerDataConfig) {
                        //服务端渲染数据
                        proxyComp.prototype.initServerDataConfig.map(function (item) {
                            //let val =item.defaultValue; //编辑预览模式, 直接取默认值 . 如果是服务器渲染
                            widgetTreeConfig = widgetTreeConfig.setIn(item.propsPath, immutable_1.fromJS(item.defaultValue));
                        });
                    }
                    if (proxyComp.prototype.transferProps) {
                        //transferProps目前不会启用了.
                        props = proxyComp.prototype.transferProps(widgetTreeConfig.get("props"));
                    } else {
                        props = widgetTreeConfig.get("props").toObject(); //浅转换
                    }
                } else {
                    props = widgetTreeConfig.get("props").toObject(); //浅转换
                }
            ...
            if (isShowFull) {
                //是否显示多余的信息出来
                (0, _assign2.default)(props, propsToChildren);
                props.dataWidgetPath = pathArray.toJS().join(",");
            }
            var childrenArray = null;
            if (widgetTreeConfig.has("children") && widgetTreeConfig.get("children").count() > 0) {
                if (widgetTreeConfig.get("children").count() == 1) {
                    //只有一个的特殊处理，不然在生成源码时的调用不一致
                    var subWidgetTreeConfig = widgetTreeConfig.getIn(["children", 0]);
                    if (subWidgetTreeConfig) {
                        childrenArray = ConfigOrderUtil.generateCompTree({
                            configOrder: subWidgetTreeConfig,
                            pathArray: pathArray.push("children").push(0),
                            propsToChildren: propsToChildren,
                            isShowFull: isShowFull,
                            isEditModel: isEditModel
                        });
                    }
                } else {
                    childrenArray = widgetTreeConfig.get("children").filter(function (childrenItem) {
                        return !!childrenItem;
                    }).map(function (subWidgetTreeConfig, index) {
                        return ConfigOrderUtil.generateCompTree({
                            configOrder: subWidgetTreeConfig,
            ...
            if (!Comp) {
                throw new Error("未根据NS定位到组件中::" + (0, _stringify2.default)(widgetTreeConfig.toJS()));
            }
            return React.createElement(Comp, props, childrenArray);
        }
        /**
         * 清除configOrder中的__Info信息;;
         * 一般用于发布页面时,剔除这些信息, 减少内容大小.
         * @param configOrder
         */

    }, {
        key: "cleanWidgetINFO",
        value: function cleanWidgetINFO(configOrder) {
            var __infoPath = [];
            ConfigOrderUtil.treeConfig({
                configOrder: configOrder,
                hitCompCalBack: function hitCompCalBack(widgetConfig, pathArray) {
                    if (widgetConfig.has("__info")) {
                        __infoPath.push(pathArray.push("__info").toJS());
                    }
                }
            });
            for (var i = 0, ilen = __infoPath.length; i < ilen; i++) {
         ...
         * 将配置分单分类 .
         * @param configOrder
         * @return {{headerBars: Array, contendBars: Array, footBars: Array, pluginBars: Array}}
         */

    }, {
        key: "getCommonConfig",
        value: function getCommonConfig(_ref2) {
            var configOrder = _ref2.configOrder;

            var widgetProxyRep = widget_center_1.default.getRepo().widgetProxyRep;
            var result = {
                headerBars: [],
                contendBars: [],
                footBars: [],
            ...
            // 相当于最顶部的
            // 只取头与尾的两个部分. 从首页中获取, 那么其他页面的这个内容呢?
            //取的规则 也要改的.
            configOrder.get("children").forEach(function (widgetBar) {
                //公共头部,
                var _prototype = widgetProxyRep[widgetBar.get("widgetNameSpace")].prototype;
                if (_prototype.isCommonHeader === true) {
                    result.headerBars.push(widgetBar);
                } else if (_prototype.isCommonFooter === true) {
                    //公共尾部.
                    result.footBars.push(widgetBar);
                } else if (_prototype.isCommonPlugin === true) {
                    //公共插件.
                    result.pluginBars.push(widgetBar);
                } else {
                    result.contendBars.push(widgetBar);
                }
            });
            return result;
        }
        /**
         *
         * 根据配置信息, 将内容转换为对应平台配置清单;;
         *
         * step1  wdgetNameSpace映射转换
         * step2  删除没有了组件
         * step3  根据__info中的值重新计算生成链接信息
         *
         *
         * @param {any} transferInfo  转换映射内容
         * @param {any} configOrder 待转换的配置内容
         * @param {any} linkEdits 对应平台的链接转换信息 注: linkkey要一致
         *
         * @returns {any}
         */

    }, {
        key: "transferDoublePlatformTpl",
        value: function transferDoublePlatformTpl(_ref3) {
            var transferInfo = _ref3.transferInfo,
                configOrder = _ref3.configOrder,
                linkEdits = _ref3.linkEdits;

            var localToOther = {},
                other2Local = {}; //本平台组件与映射平台组件 . 映射平台组件与本平台组件
            if (!(configOrder instanceof immutable_1.Map)) {
                configOrder = immutable_1.fromJS(configOrder);
            }
            if (!(transferInfo instanceof immutable_1.Map)) {
                transferInfo = immutable_1.fromJS(transferInfo);
            }
            var _weixinCo = (0, _stringify2.default)(configOrder.toJS());
            transferInfo.get("barMap", []).forEach(function (toNs, loNs) {
                localToOther[loNs] = toNs;
                other2Local[toNs] = loNs;
                _weixinCo = _weixinCo.split(loNs).join(toNs);
            });
            transferInfo.get("groupMap", []).forEach(function (toNs, loNs) {
                localToOther[loNs] = toNs;
                other2Local[toNs] = loNs;
                _weixinCo = _weixinCo.split(loNs).join(toNs);
            });
            _weixinCo = immutable_1.fromJS(JSON.parse(_weixinCo));
            //1. 记录本配置有而对应平台配置清单没有的 组件及索引位置,这些要删除的..
            //2. 记录本清单没有而对应配置清单有的,组件及索引位置,这些要保留..
            //   例子
            //   头部 尾部
            //   微信的logo标志
            //	 PC有插件 而微信没有插件 .
            var localOnlyBarArray = []; //这些是需要删除的, 只需要记录组件index 就可以了.
            configOrder.get("children").forEach(function (childWidgetItem, _index) {
                //如果映射中不包含这个组件
                if (!localToOther[childWidgetItem.get("widgetNameSpace")]) {
                    localOnlyBarArray.push(_index);
                }
            });
            //倒排序一次. 从后往前删,删除对应对台没有的组件
            for (var i = localOnlyBarArray.length - 1; i >= 0; i--) {
                var toDelBarIndex = localOnlyBarArray[i];
                console.log("删除组件位置 ::", toDelBarIndex);
                _weixinCo = _weixinCo.deleteIn(["children", toDelBarIndex]);
            }
            //记录需要处理的地方
            var toDealPathList = [];
            ConfigOrderUtil.treeConfig({
                configOrder: _weixinCo,
                hitCompCalBack: function hitCompCalBack(widgetTreeConfig, pathArray) {
                    widgetTreeConfig.has("__info") ? toDealPathList.push(pathArray.toJS()) : null;
                }
            });
            //link类型的影射;;
            var _linkTypeMap = {};
            for (var _i = 0, _ilen = linkEdits.length; _i < _ilen; _i++) {
                var item = linkEdits[_i];
                _linkTypeMap[item.linkKey] = item;
            }
            //link属性如果检测,如果改变了, 要处理的..
            toDealPathList.forEach(function (widgetPathArray) {
                var __info = _weixinCo.getIn(widgetPathArray.concat(["__info"]));
                __info.forEach(function (infoItem, propsPathItem) {
                    if (infoItem.get && infoItem.get("type") === "link" && infoItem.has("info") && infoItem.has("linkKey")) {
                        var propsPath = widgetPathArray.concat(propsPathItem.split(","));
                        //按照 新的方案, 这里 是不用转换的...  要朝着这个方向走了.
                        var _linkKey = infoItem.get("linkKey");
                        if (!_linkTypeMap[_linkKey]) {
                            throw new Error("systemConfig.${platform}.linkEdit不包含linkKey::" + _linkKey);
                        }
                        if (infoItem.has("info") && infoItem.get("info") && infoItem.get("info").toJS) {
                            var _href = output_generate_1.evalLinkOutPut({
                                info: infoItem.get("info").toJS(),
                                linkConfigItem: _linkTypeMap[_linkKey]
                            });
                            if (_weixinCo.hasIn(propsPath)) {
                                _weixinCo = _weixinCo.setIn(propsPath, _href);
                            }
                        }
                    }
                });
        ...
        /**
         * 遍历配置清单中的内容
         * @param widgetTreeConfig 组件配置.
         * @param hitComp  命中后的处理.
         * @param pathArray 组件路径
         */

    }, {
        key: "treeConfig",
        value: function treeConfig(_ref4) {
            var configOrder = _ref4.configOrder,
                hitCompCalBack = _ref4.hitCompCalBack,
                _ref4$pathArray = _ref4.pathArray,
                pathArray = _ref4$pathArray === undefined ? immutable_1.List() : _ref4$pathArray;
            ...
            if (configOrder.has("children") && configOrder.get("children").count() > 0) {
                configOrder.get("children").forEach(function (subWidgetTreeConfig, index) {
                    return ConfigOrderUtil.treeConfig({
                        configOrder: subWidgetTreeConfig,
                        hitCompCalBack: hitCompCalBack,
                        pathArray: pathArray.push("children").push(index)
                    });
                });
            }
...
$export($export.S + $export.F * !__webpack_require__(18), 'Object', { defineProperty: __webpack_require__(27).f });
...
var ReactDOMServer = __webpack_require__(211);
 ...
 * @Date    2017/9/6
 **/
function getLinkItem(_ref) {
    var linkItems = _ref.linkItems,
        linkKey = _ref.linkKey;

    var linkItem = null;
    linkItems.linkEdit.forEach(function (item) {
...
exports.getLinkItem = getLinkItem;
/**
 *
 * @param {any} linkKey
 * @param {any} info
 ...
 * @returns {any}
 */
function evalLinkOutPut(_ref2) {
    var info = _ref2.info,
        linkConfigItem = _ref2.linkConfigItem;

    var linkItem = linkConfigItem;
    if (linkItem.outputType === 'JSONStr' || linkItem.outputType === 'JSON') {
        var output = linkItem.outputLink.split("'").join('').split('"').join('');
        var objItems = output.split(','); //
        var result = {};

        var _loop = function (i) {
            var objItemArray = objItems[i].trim().split(':');
            var _name = objItemArray[0],
                _value = objItemArray[1];
            if (_value.indexOf('${') >= 0) {
                //从对像中取值填充进来
                linkItem.info.split(',').forEach(function (item) {
                    var key = item.trim();
                    if (key.indexOf('::') >= 0) {
        ...
        for (var i = 0, ilen = objItems.length; i < ilen; i++) {
            _loop(i, ilen);
        }
        if (linkItem.outputType === 'JSONStr') {
            //这个现在不需要了.
            var _result = (0, _stringify2.default)(result);
            // console.log('JSONStr',_result );
            return _result;
        } else {
            return result;
        }
    } else {
        var _output = linkItem.outputLink;
        linkItem.info.split(',').forEach(function (item) {
            var key = item.trim();
            if (key.indexOf('::') >= 0) {
                key = key.split('::')[0].trim();
            }
            var value = info[key] ? info[key] : '';
            if (linkItem.options && linkItem.options.encodeURIComponent) {
                value = info[key] ? encodeURIComponent(info[key]) : '';
            }
            _output = _output.split('${' + key + '}').join(value);
        });
        var spstr = _output.split('');
...
exports.evalLinkOutPut = evalLinkOutPut;
/**
 ...
 * @param originObj  {}
 */
function extraLinkInfo(_ref3) {
    var info = _ref3.info,
        originObj = _ref3.originObj;

    var result = {};
    var _keyInfo = info.trim().split(',');
    var _noFindKey = [];
    for (var i = 0, ilen = _keyInfo.length; i < ilen; i++) {
        //原对象属性,   复制到属性
        var key = _keyInfo[i],
            toKey = _keyInfo[i];
        if (_keyInfo[i].indexOf('::') > 0) {
            var __temp = _keyInfo[i].split('::');
            toKey = __temp[0].trim();
            key = __temp[1].trim();
        }
        if (key.indexOf('.') >= 0) {
            ...
            var _temp = originObj;
            for (var _i = 0, _ilen = keysArray.length; _i < _ilen; _i++) {
                _temp = _temp[keysArray[_i]];
            }
            result[toKey] = _temp;
...
exports.extraLinkInfo = extraLinkInfo;
...
exports.default = function (callback) {
    if (window.Promise) {
        ...
        var defererd = new $.Deferred();
        callback(defererd.resolve, defererd.reject);
...
var urlHelper = {
    parseUrlParam: function parseUrlParam() {
        //加了.replace(/\+/g, '%20')，因为url的+是空格但是decodeURI解析不了
        var _search = decodeURI(location.search.trim().replace(/\+/g, '%20')).substr(1);
        return this.str2KeyMap(_search);
    },
    ...
    /**
       ...
       * @returns {{}}
       */
    str2KeyMap: function str2KeyMap(_search) {
        var result = {};
        if (_search && _search.indexOf('=') > 0) {
            var unitItems = _search.split('&');
            unitItems.forEach(function (unitItem) {
                var tmp = unitItem.split('=');
                result[tmp[0]] = tmp[1];
            });
        }
        return result;
...
var isHttps = location.protocol === "https:"; /**
                                               ...
                                               **/
exports.default = {
    loaderJsFile: function loaderJsFile(jsSrc) {
        console.warn("建议使用xDllContext.$script");
        if (isHttps) {
            //加 //就可以. 但是app中不识别这种情况 不行. app中//是从本地读文件的
            jsSrc = jsSrc.replace("http://", "https://");
        }
        // log('加载JS文件',jsSrc);
    ...
    loaderCssFile: function loaderCssFile(cssSrc) {
        (0, _log2.default)("加载CSS文件", cssSrc);
        if (isHttps) {
            //加 //就可以. 但是http 不行.
            cssSrc = cssSrc.replace("http://", "https://");
        }
        return new _promise2.default(function (resolve, reject) {
            ...
            node.onload = function () {
                (0, _log2.default)("Css加载成功. ", cssSrc);
                resolve();
            };
            node.onerror = function (err) {
                (0, _log2.default)("Css加载失败!. ", cssSrc);
                reject(err);
            };
 ...
 * @Date    16/8/29
 **/
var pcHeadCommon = '.x-head-1 {display: block;}.x-head-2 {display: block;}.x-head-3 {display: block;}';
var pcFootCommon = '.x-foot-1 {display: block;}.x-foot-2 {display: block;}.x-foot-3 {display: block;}';

var displayRuleUtil = function displayRuleUtil() {
    (0, _classCallCheck3.default)(this, displayRuleUtil);
};
...
displayRuleUtil.getHitRules = function (url, ruleArray) {
    var hitRules = [];
    for (var i = 0, ilen = ruleArray.length; i < ilen; i++) {
        var _ruleInfo = ruleArray[i];
        if (_ruleInfo.type === 'string') {
            var _index = url.indexOf(_ruleInfo.url);
...
displayRuleUtil.getFirstHitRule = function (url, ruleArray) {
    var relues = displayRuleUtil.getHitRules(url, ruleArray);
...
displayRuleUtil.getRuleCssContent = function (ruleInfo) {
    ...
    if (ruleInfo.footLv >= 0) {
        var footCss = pcFootCommon;
        for (var _i = 1; _i < 3; _i++) {
            if (_i > ruleInfo.footLv) {
                footCss = footCss.replace('.x-foot-' + _i + ' {display: block;}', '.x-foot-' + _i + ' {display: none;}');
            }
        }
 ...
 * @private
 */
function _generateRultItem(_ref) {
    var url = _ref.url,
        headLv = _ref.headLv,
        footLv = _ref.footLv,
        _ref$type = _ref.type,
        ...
        operate = _ref$operate === undefined ? 'beginWith' : _ref$operate;
...
exports.getPicSrc = getPicSrc;

var _envConfig = __webpack_require__(254);

var _imageOptimize = __webpack_require__(255);

var _imageOptimize2 = _interopRequireDefault(_imageOptimize);
 ...
 * image-optimize.ts
 * @authors 杨兴洲（of2502）
 * @date    2017/5/5 0005 9:30
 * @version 1.0
 * @last-modify  杨兴洲（of2502）
 */
var imageOptimize = new _imageOptimize2.default({
    devicePixelRatio: window.devicePixelRatio || 1,
    isSupportWebp: _envConfig.isSupportWebp,
    isHttps: _envConfig.isHttps
});
/**
 * 生成src,根据width
 * <img src={imgSrc}>
 ...
 * @returns {string}
 * @constructor
 * @deprecated 这里的方法尽快改为image-optimize 的调用 ...
 */
function createSrc(_ref) {
...
/**
 * 生成srcset
 * "img.jpg@100w 1x, img.jpg@200w 2x, img.jpg@300w 3x"
 * @param imgSrc
 * @param w
 * @param h
 * @returns {string}
 */
function createSrcset(_ref2) {
    var imgSrc = _ref2.imgSrc,
        w = _ref2.w,
        h = _ref2.h;

    if (!w && !h) return null; // 没有宽高,没发设置多种尺寸。不处理
    return [1, 2, 3].map(function (dpr) {
        return createSrc({ imgSrc: imgSrc, w: w, h: h, dpr: dpr }) + ' ' + dpr + 'x';
    }).join(',');
}
var imageFormats = ['.jpg', '.JPG', '.jpeg', '.JPEG', '.webp', '.WEBP', '.png', '.PNG', '.gif', '.GIF', '.blob', '.BLOB', '.timg', '.TIMG'];
function isImageFormat(_ref3) {
    var imageSrc = _ref3.imageSrc;

    //去除图片处理标识...
    if (imageSrc.lastIndexOf('@') >= 0) {
        var atIndex = imageSrc.lastIndexOf('@');
        imageSrc = imageSrc.substr(0, atIndex);
    }
    for (var i = 0, ilen = imageFormats.length; i < ilen; i++) {
        var _index = imageSrc.lastIndexOf(imageFormats[i]);
        if (_index >= 0 && _index + imageFormats[i].length === imageSrc.length) {
            return true;
        }
    }
    return false;
 ...
 * @returns {any}
 */
function getPicSrc(imgSrc, width, height, options) {
    //photo.jpeg ==> photo-50-50.jpeg
    if (!imgSrc) {
        return imgSrc;
    }
    //{isSuportWebp}
    var isSuportWebp = options && options.isSuportWebp;
    if (imgSrc.lastIndexOf('@') >= 0) {
        var atIndex = imgSrc.lastIndexOf('@');
        imgSrc = imgSrc.substr(0, atIndex);
    }
    var pointIndex = imgSrc.lastIndexOf('.');
    var afterPre = imgSrc.substr(pointIndex, imgSrc.length);
    console.log(afterPre);
    //取文件格式
    var _fileFormat = ''; // 若不是文件格式 那么最后就不加了.
    for (var i = 0, ilen = imageFormats.length; i < ilen; i++) {
        if (afterPre.toLowerCase().indexOf(imageFormats[i]) >= 0) {
            _fileFormat = imageFormats[i];
            break;
        }
    }
    //取文件名前的一段
    var preImageContent = imgSrc.substr(0, pointIndex);
    var fileName = preImageContent.substr(preImageContent.lastIndexOf('/'));
    if (imgSrc.indexOf('www.qmimg.com') >= 0 && width && height) {
        if (fileName.indexOf('-') >= 0) {
            //排除, 原先有压缩的情况发生
            preImageContent = preImageContent.replace(fileName, fileName.substr(0, fileName.indexOf('-')));
        }
        return preImageContent + '-' + width + '-' + height + '-f' + _fileFormat;
    } else if (imgSrc.indexOf('img.1000.com') >= 0 && (width || height)) {
        //阿里云
        var result = preImageContent + afterPre + ('@' + (width ? width + 'w' : '') + (height ? '_' + height + 'h' : '') + '_1wh' + _fileFormat.toLowerCase());
        return isSuportWebp ? result + '.webp' : result;
    } else if (imgSrc.indexOf('web-img.qmimg.com') >= 0 && (width || height)) {
        //阿里云
        var _result = preImageContent + afterPre + ('@' + (width ? width + 'w' : '') + (height ? '_' + height + 'h' : '') + '_1wh' + _fileFormat.toLowerCase());
        return isSuportWebp ? _result + '.webp' : _result;
    } else if (imgSrc.indexOf('oss-hz.qianmi.com/x-site') >= 0 && (width || height)) {
        //阿里云
        var _result2 = preImageContent.replace('oss-hz.qianmi.com/x-site', 'web-img.qmimg.com/x-site') + afterPre + ('@' + (width ? width + 'w' : '') + (height ? '_' + height + 'h' : '') + '_1wh' + _fileFormat.toLowerCase());
        return isSuportWebp ? _result2 + '.webp' : _result2;
    } else {
        return imgSrc;
    }
}
...
if (document) {
    ...
    var _img = _canvas.toDataURL('image/webp');
    if (_img) {
        exports.isSupportWebp = isSupportWebp = _img.indexOf('data:image/webp') === 0;
    }
    exports.isEditEnv = isEditEnv = !!window.XSite_Info; // 是否是编辑环境
    exports.isHttps = isHttps = window.location.protocol === 'https:';
}
exports.isSupportWebp = isSupportWebp;
exports.isEditEnv = isEditEnv;
 ...
 * @Date    2017/8/20
 **/
var OssCDNList = ['web-img.qmimg.com', 'img.1000.com'];

var ImageOptimize = function () {
    function ImageOptimize(_ref) {
        var _ref$devicePixelRatio = _ref.devicePixelRatio,
            devicePixelRatio = _ref$devicePixelRatio === undefined ? 1 : _ref$devicePixelRatio,
            _ref$isSupportWebp = _ref.isSupportWebp,
            isSupportWebp = _ref$isSupportWebp === undefined ? false : _ref$isSupportWebp,
            _ref$isHttps = _ref.isHttps,
            isHttps = _ref$isHttps === undefined ? false : _ref$isHttps;
        (0, _classCallCheck3.default)(this, ImageOptimize);
    ...
    (0, _createClass3.default)(ImageOptimize, [{
        key: 'optimize',
        value: function optimize(_ref2) {
            var imgSrc = _ref2.imgSrc,
                width = _ref2.width,
                height = _ref2.height,
                _ref2$devicePixelRati = _ref2.devicePixelRatio,
                devicePixelRatio = _ref2$devicePixelRati === undefined ? this.devicePixelRatio : _ref2$devicePixelRati;

            if (!imgSrc) {
                imgSrc = '';
            }
            // https处理
            if (this.isHttps) imgSrc = imgSrc.replace('http://', 'https://');
            // 【有宽或高】自己处理 imgSrc.split("@")[0] + suffix
            // 【无宽高|未处理过|支持webp】自己处理  imgSrc + `@${type}`;
            // 【无宽高|未处理过|不支持webp】不作处理
            // 【无宽高|已处理过】不作处理
            var isGif = /gif/.test(imgSrc); // gif 不处理
            var isOssCDN = OssCDNList.some(function (item) {
                return imgSrc.indexOf(item) > -1;
            }); // 图片是否有oss处理能力
            var isHandled = /@(\d+[wh]|\.webp)/.test(imgSrc); // 图片是否处理过。含有 @100w_100h ||  @100w_1wh || @.webp 表示处理过
            var originType = '.' + imgSrc.split('@')[0].split('.').pop().toLowerCase(); // 不加后缀.会默认处理成jpg.导致颜色被填充
            var type = this.isSupportWebp ? isGif ? '.gif' : '.webp' //gif 后缀不处理
            : originType === '.webp' ? '.jpg' : originType; // 图片格式处理
            if (!isOssCDN) return imgSrc; // 不能处理。直接返回
            if (width || height) {
                var suffix = '@';
                if (width) {
                    suffix += Math.ceil(width * devicePixelRatio) + 'w';
                    if (height) suffix += '_' + Math.ceil(height * devicePixelRatio) + 'h';
                } else {
                    suffix += Math.ceil(height * devicePixelRatio) + 'h';
                }
                suffix += '_1l_1wh' + type; // 图片宽高处理后缀
                imgSrc = imgSrc.split('@')[0] + suffix;
            } else {
                if (!isHandled && type) imgSrc = imgSrc.split('@')[0] + ('@' + type);
            }
            return imgSrc;
        }
    }]);
...
exports.msg = exports.default = undefined;
...
var _mitt = __webpack_require__(257);

var mitt = _interopRequireWildcard(_mitt);
 ...
 * @Date    2017/3/29
 **/
var XSiteMsg = function () {
    function XSiteMsg() {
        (0, _classCallCheck3.default)(this, XSiteMsg);

        this.emitter = new (mitt.default || mitt)();
    }

    (0, _createClass3.default)(XSiteMsg, [{
        key: 'on',
        value: function on(type, handler) {
            (0, _log2.default)('[事件] 监听::', type, handler);
        ...
        value: function off(type, handler) {
            (0, _log2.default)('[事件] 解除监听::', type, handler);
        ...
        value: function once(type, handler) {
            ...
            (0, _log2.default)('[事件] 监听一次::', type, handler);
            var _onceFunc = function (arg) {
                handler(arg);
                _this.emitter.off(type, _onceFunc);
            };
            this.emitter.on(type, _onceFunc);
            return this;
        }
    }, {
        key: 'removeListener',
        ...
        value: function emit(type, event) {
            (0, _log2.default)('发送事件:', type, event);
            if (arguments.length > 2) {
                console.error('参数过多,仅支持一个参数' + type + event);
            }
...
//目前编辑没有使用openApi全部代替, 所以 window['xDllContext'] 会加载两次.


exports.default = XSiteMsg;
var _msg = void 0;
if (window['xDllContext'] && window['xDllContext']['msg']) {
    _msg = window['xDllContext']['msg'];
} else {
    _msg = new XSiteMsg();
    window['xDllContext'] = window['xDllContext'] || {};
    window['xDllContext']['msg'] = _msg;
    window['xDllContext']['iflux'] = { msg: _msg };
}
var msg = exports.msg = _msg;
 ...
 * @Date    16/11/1
 *
 * @deprecated 移到dll中.. 方便其他地方 引用 ..
 **/

var React = __webpack_require__(40);
var a_comp_1 = __webpack_require__(136);
var x_site_kit_1 = __webpack_require__(50);
var XImgWeixin = /** @class */function (_super) {
    __extends(XImgWeixin, _super);
    function XImgWeixin(props) {
        return _super.call(this, props) || this;
    }
    XImgWeixin.prototype.render = function () {
        var _a = this.props,
            w = _a.w,
            h = _a.h,
            width = _a.width,
            ...
            imgSrc = _a.imgSrc,
            imgHref = _a.imgHref,
            enableLazyLoad = _a.enableLazyLoad;
        var _style = __assign({}, style, { width: width, height: height });
        // 优化图片
        var src = x_site_kit_1.optimizeImage.createSrc({ w: w, h: h, imgSrc: imgSrc, dpr: 1 });
        var srcSet = x_site_kit_1.optimizeImage.createSrcset({ w: w, h: h, imgSrc: imgSrc });
        // 是否懒加载
        var image = enableLazyLoad ? React.createElement("img", { key: src, className: "ImgComp lazyload", "data-src": src, "data-srcset": srcSet, style: _style }) : React.createElement("img", { key: src, className: "ImgComp", src: src, srcSet: srcSet, style: _style });
        // 有无链接
        return imgHref !== undefined ? React.createElement(a_comp_1.default, { href: imgHref, target: this.props.target }, image) : { image: image };
    };
    XImgWeixin.defaultProps = {
        enableLazyLoad: true,
        lazyImageSrc: "http://web-img.qmimg.com/x-site/public/images/pro-list/lazy-img.png",
        imgSrc: "",
        imgHref: "",
        target: "_self"
    };
...
module.exports = {
  set: Object.setPrototypeOf || ('__proto__' in {} ? // eslint-disable-line
    function (test, buggy, set) {
      try {
        set = __webpack_require__(26)(Function.call, __webpack_require__(264).f(Object.prototype, '__proto__').set, 2);
        set(test, []);
...
/**
 * 和下面两个文件内容基本一样：
 * 1. @qianmi/x-site-ui-base 中的 ImgComp (注意目录结构，@qianmi/x-site-ui-base 中有两个 webp，这是 weixin 下面的)
 * 2. ./img-comp.tsx
 * 该文件的增量为会使用 webp
 */
var XImgWeixin = /** @class */function (_super) {
    ...
    XImgWeixin.prototype.render = function () {
        ...
        var src = x_site_kit_1.optimizeImage.createSrc({ w: w, h: h, imgSrc: imgSrc, dpr: 1 });
        var srcSet = x_site_kit_1.optimizeImage.createSrcset({ w: w, h: h, imgSrc: imgSrc });
        var srcWebp = x_site_kit_1.optimizeImage.createSrc({
            w: w,
            h: h,
            imgSrc: imgSrc,
            dpr: 1,
        ...
        var srcSetWebp = x_site_kit_1.optimizeImage.createSrcset({ w: w, h: h, imgSrc: imgSrc, webp: true });
        // 是否懒加载
        var image = enableLazyLoad ? React.createElement("picture", null, React.createElement("source", { type: "image/webp", "data-src": srcWebp, "data-srcset": srcSetWebp }), React.createElement("source", { "data-src": src, "data-srcset": srcSet }), React.createElement("img", { key: src, className: "ImgComp lazyload", "data-src": src, "data-srcset": srcSet, style: _style })) : React.createElement("picture", null, React.createElement("source", { type: "image/webp", src: srcWebp, srcSet: srcSetWebp }), React.createElement("source", { src: src, srcSet: srcSet }), React.createElement("img", { key: src, className: "ImgComp", src: src, srcSet: srcSet, style: _style }));
        // 有无链接
        return imgHref !== undefined ? React.createElement(a_comp_1.default, { href: imgHref, target: target }, image) : { image: image };
    };
    XImgWeixin.defaultProps = {
        enableLazyLoad: true,
        lazyImageSrc: "http://web-img.qmimg.com/x-site/public/images/pro-list/lazy-img.png",
...
/**
 * 云商城
 * http://git.dev.qianmi.com/gcs/wx-d2c/blob/br-iflux/webapp/assets/view/apps/app.js
 */
...
Object.defineProperty(exports, "__esModule", { value: true });
var x_site_kit_1 = __webpack_require__(50);
var BusinessDataCenter = /** @class */function () {
    function BusinessDataCenter() {}
    /**
     * 调用 业务线Api
     * @param fnName    require Api方法
     * @param data      option Api参数
     * @returns {Promise}
     */
    BusinessDataCenter.getResult = function (fnName, data) {
        return new _promise2.default(function (resolve, reject) {
            BusinessDataCenter.getData(fnName, data, function (result) {
                resolve(result);
            }, function (err) {
                reject(err);
            });
    ...
    BusinessDataCenter.getData = function (fnName, data, success, fail) {
        if (BusinessDataCenter.ApiCenter) {
            //API已注册 直接处理请求
            BusinessDataCenter.dealReq(fnName, data, success, fail);
        } else {
            //API未注册
            x_site_kit_1.log("API未注册,延迟等待执行!", fnName, data);
            delayExecute({
                decideFunc: function () {
                    return !!BusinessDataCenter.ApiCenter;
                },
                okFunc: function () {
                    x_site_kit_1.log("API已成功绑定,执行方法:", fnName, data);
                    BusinessDataCenter.dealReq(fnName, data, success, fail);
                }
            });
        }
    };
    BusinessDataCenter.dealReq = function (fnName, data, success, fail) {
        if (!BusinessDataCenter.ApiCenter[fnName]) {
            x_site_kit_1.log("调用apiCenter方法失败:", fnName, data);
            throw new Error("\u63A5\u53E3" + fnName + "\u672A\u5B9A\u4E49");
        }
        if (typeof BusinessDataCenter.ApiCenter[fnName] !== "function") {
            x_site_kit_1.log("调用apiCenter方法失败:", fnName, data);
            throw new Error(fnName + "\u63A5\u53E3\u7C7B\u578B\u6709\u8BEF");
        }
        var res = null;
        try {
            res = BusinessDataCenter.ApiCenter[fnName](data);
            x_site_kit_1.log("调用apiCenter方法返回值:", fnName, data, res);
            success && success(res);
        } catch (e) {
            x_site_kit_1.log("调用apiCenter方法失败:", fnName, data, e);
            fail && fail(e);
        }
    };
    return BusinessDataCenter;
}();
exports.default = BusinessDataCenter;
x_site_kit_1.msg.once("Inject:Api:To:xSite", function (api) {
    if (!BusinessDataCenter.ApiCenter) {
        BusinessDataCenter.ApiCenter = api;
        x_site_kit_1.log("成功通过事件[Inject:Api:To:xSite]注入api对象:", api);
    }
});
/**
 * 延迟执行方法.
 * @param decideFunc  判断方法
 * @param okFunc
 */
function delayExecute(_a) {
    var decideFunc = _a.decideFunc,
        okFunc = _a.okFunc;
    if (!decideFunc()) {
        setTimeout(delayExecute.bind(null, { decideFunc: decideFunc, okFunc: okFunc }), 200);
    } else {
        okFunc();
    }
}
