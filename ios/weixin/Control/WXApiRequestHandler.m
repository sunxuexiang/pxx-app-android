...
+ (void)jumpToBizPay:(NSDictionary *) payReq
              callback:(RCTResponseSenderBlock)callback
{
  ...
  NSMutableString *stamp  = [payReq objectForKey:@"timestamp"];
  //调起微信支付
  PayReq* req   = [[[PayReq alloc] init]autorelease];
  req.partnerId = [payReq objectForKey:@"partnerid"];
  req.prepayId  = [payReq objectForKey:@"prepayid"];
  req.nonceStr  = [payReq objectForKey:@"noncestr"];
  req.timeStamp = stamp.intValue;
  req.package   = [payReq objectForKey:@"package"];
  req.sign      = [payReq objectForKey:@"sign"];
  ...
  //日志输出
  NSLog(@"appid=%@\npartid=%@\nprepayid=%@\nnoncestr=%@\ntimestamp=%ld\npackage=%@\nsign=%@",[payReq objectForKey:@"appid"],req.partnerId,req.prepayId,req.nonceStr,(long)req.timeStamp,req.package,req.sign );
  
  [WXApi sendReq:req completion:^(BOOL success) {
    if(!success) {
      if(callback != nil){
        callback(@[@"微信支付失败，请选择其他支付方式", [NSNull null]]);
      }
    }
  }];
}
