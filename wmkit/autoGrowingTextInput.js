...
export default class AutoGrowingTextInput extends Component {
  constructor(props) {
    ...
    this._onContentSizeChangeAndroid = this._onContentSizeChangeAndroid.bind(
      this
    );
  ...
  shouldApplyNativeIOSSettings() {
    return (
      ...
      (this.props.disableScrollAndBounceIOS ||
        this.props.enableScrollToCaretIOS)
    );
  }
    ...
    return (
      <View style={{ flex: 1, flexDirection: 'row' }}>
        <TextInput
          ...
          style={[
            this.props.style,
            { height: this._getValidHeight(this.state.height), flex: 1 }
          ]}
  ...
  render() {
    return ANDROID_PLATFORM
      ? this._renderTextInputAndroid()
      : this._renderTextInputIOS();
  }
