...
export default class AppStore extends Store {
  ...
  _testName = (name) => {
    if (name === '') {
      msg.emit('app:tip', '请填写称呼！');
      return false;
    } else if (name.length < 2 || name.length > 15) {
      msg.emit('app:tip', '称呼为2-15个字符！');
      return false;
    }
    return true;
  };
  ...
  init = async () => {
    ...
    if (
      ...
      this.dispatch('enterprise: registerType : init');
      //初始化企业注册协议
      if (res[2]) {
        this.dispatch(
  ...
  doLogin = async () => {
    ...
    const Check = this.state().get('isCheck');
    let result = null;
    if (isALogin) {
      /**获取用户名和密码，并去除所有空格*/
      ...
      if (WMkit.testTel(account) && WMkit.testPass(password)) {
        if (Check) {
          ...
          account.length !== 0 && password.length !== 0 && msg.emit('app:tip', '请先阅读并同意相关协议');
          return null;
        }
      }else{
        return null;
      ...
      if (
        ...
        account.length !== 0 && verificationCode.length !== 0 && msg.emit('app:tip', '请先阅读并同意相关协议');
        return null;
      }
    }
    ...
    if (result.code == config.SUCCESS_CODE) {
      ...
      if (context.checkState == 1){
        msg.emit('user:center:refresh');
      }
  ...
  onChangeBox = () => {
    this.dispatch('login:isCheck')
  }
  initCheck=()=>{
    this.dispatch('login:initCheck')
  }
  /**
   * 输入验证码
   * @param code
    ...
    if (
      ...
      if(Check) {
        ...
        if (code == config.SUCCESS_CODE) {
          ...
          await this.registerInfo(registerInfo);
          return true;
  ...
  doEnterpriseRegister = async () => {
    ...
    if (
      ...
      mobile.length !== 0 && inviteCode.length !== 0 &&
        sendCode.length !== 0 &&
        password.length !== 0 && msg.emit('app:tip', '请先阅读并同意相关协议')
      return false;
    }
  };
