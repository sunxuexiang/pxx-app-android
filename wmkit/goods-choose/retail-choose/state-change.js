 ...
 * @private
 */
export const returnCartStockFlag = (stock) => {
  //认定缺货的状态 >>> 库存小于等于0
  if (isNaN(stock)) {
    return true;
  }
 ...
 * 删除商品
 */
export const deletePurchase = async (goodsInfoId) => {
  if (WMkit.isLoginOrNotOpen()) {
    const { code } = await webapi.deletePurchaseCount(goodsInfoId);
    if (code !== config.SUCCESS_CODE) {
      msg.emit('app:tip', '删除失败，请重试！');
      return;
    }
  } else {
    const flag = await delPurchase(skuIds.toJS());
    if (!flag) {
      msg.emit('app:tip', '删除失败，请重试！');
      return;
    }
 ...
 * 立刻抢购
 */
export const rushToBuyingFlashSaleGoodsInfo = async (flashSaleGoodsId, num) => {
  if (WMkit.isLoginOrNotOpen()) {
...
export const changePurchase = async (oldGoodsInfoId, buyGoodsInfo) => {
  if (WMkit.isLoginOrNotOpen()) {
    const { code: delCode } = await webapi.deletePurchaseCount(oldGoodsInfoId);
    if (delCode !== config.SUCCESS_CODE) {
      msg.emit('app:tip', '删除失败，请重试！');
      return;
    }
    ...
    const goodsInfo = {
      ...
      buyCount: buyGoodsInfo.get('num') ? buyGoodsInfo.get('num') : 1
    };

    const { code, message } = await webapi.purchaseBatch([goodsInfo]);
    if (code == config.SUCCESS_CODE) {
      msg.emit('app:tip', '加入成功');
    } else {
      msg.emit('app:tip', message);
